...
BOOL NoExceptions_Enter()
{
	...
	__try
	{
		if (cpustate == NULL)
		{
			...
			cpustate = ExAllocatePool2(NonPagedPool, MaxCPUCount*sizeof(CPUSTATE), 'tag');
			...
			if (cpustate)
			{
				...
				{
					cpustate[i].NoExceptionVectorList = ExAllocatePool2(NonPagedPool, 256 * sizeof(INT_VECTOR), 'tag');
