...
*/
{
	...
	if (s == STATUS_SUCCESS)
	{
		...
		if (fsi.EndOfFile.HighPart == 0)
		{
			...
			*buffer = ExAllocatePool2(POOL_FLAG_PAGED, fsi.EndOfFile.LowPart, 'tag');	 //caller MUST free this
	...
	if (s == STATUS_SUCCESS)
	{
		...
		if (s == STATUS_SUCCESS)
		{
			...
			if (s == STATUS_SUCCESS)
			{
				...
				PUCHAR pbHashObject = ExAllocatePool2(POOL_FLAG_PAGED, objectlength, 'tag');
				...
				if (pbHashObject)
				{
					...
					if (s == STATUS_SUCCESS)
					{
						...
						if (s == STATUS_SUCCESS)
						{
							...
							if (s == STATUS_SUCCESS)
							{
								PUCHAR pbHashBuffer = ExAllocatePool2(POOL_FLAG_PAGED, hashlength, 'tag');
								if (pbHashBuffer)
								{
									...
									ExFreePool2(pbHashBuffer, 'tag', NULL, 0);
								}
								else
					...
					ExFreePool2(pbHashObject, 'tag', NULL, 0);
				}
				else
